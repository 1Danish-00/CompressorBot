...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -c:v libx264 -metadata title=krisp -pix_fmt yuv420p -preset medium -tune film -s 800x520 -crf 30 -b:v 600k -c:a libopus -ac 2 -vbr 2  -ab 30k -c:s copy -map 0:s? "{out}" -y'
        process = await asyncio.create_subprocess_shell(
...
async def customenc(e, key):
    ...
    cmd = f'ffmpeg -i "{dl}" -preset fast -c:v libx264 -crf 32 -map 0:v -c:a libopus -ac 2 -ab 40k -vbr 2  -map 0:a -c:s copy -map 0:s? "{out}" -y'
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()

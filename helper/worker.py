...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -preset veryfast -c:v libx264 -crf 32 -map 0:v -map 0:a -c:s copy -map 0:s? -pix_fmt yuv420p -c:a libopus -profile:a aac_he_v2 -ac 2 -vbr 2 -ab 40k "{out}" -y'
        process = await asyncio.create_subprocess_shell(

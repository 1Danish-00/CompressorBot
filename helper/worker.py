...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -preset medium -c:v libx265 -crf 29 -map 0:v -c:a libopus -map 0:a -c:s copy -map 0:s? "{out}" -y'
        process = await asyncio.create_subprocess_shell(
...
async def sample(e):
    ...
    ncmd = f'ffmpeg -i "{dl}" -preset medium -ss {ss} -to {dd} -c:v libx265 -crf 29 -map 0:v -c:a libopus -vbr 2 -map 0:a -c:s copy -map 0:s? "{out}" -y'
    process = await asyncio.create_subprocess_shell(
        ncmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
...
async def customenc(e, key):
    ...
    cmd = f'ffmpeg -i "{dl}" -preset medium -c:v libx265 -crf 29 -map 0:v -c:a libopus -vbr 2 -map 0:a -c:s copy -map 0:s? "{out}" -y'
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
